...
$$
AP =\frac{1}{N_{p}} \sum_{i=1}^{N_{p}} \frac{|\{k \mid Rank(p_{k}) \leq Rank(p_{i})\}|}{Rank(p_{i})}\tag 9
$$

$$
HR @ K =\frac{|\{k \mid Rank(p_{k}) \leq K\}|}{K}\tag{10}
$$
...
​	其中$r_i$是新闻与第 i 个排名的相关性得分，点击新闻为 1，非点击新闻为 0。还有其他几个指标，例如点击率 (CTR) 和停留时间，可以是仅用于衡量在线新闻推荐器的性能。
...
​	通过对现有新闻推荐技术的不同方面的综合回顾，我们可以看到个性化新闻推荐技术在过去几年中取得了长足的进步。 然而，仍然存在许多挑战和未解决的问题。 因此，在本节中，我们提出了几个值得未来探索的潜在方向。

### 10.1 深度新闻理解

​	新闻建模是个性化新闻推荐的核心。可以在以下几个方面进行改进。首先，文本理解是新闻建模中的核心问题，现有方法可能无法深入理解新闻的文本内容。因此，使用更先进的 NLP 技术（例如，知识感知 PLM）可能有助于更好地理解新闻文本并改进新闻建模。其次，除了文本信息，新闻还包含丰富的多模态信息，如图像、视频和幻灯片。多模式新闻内容可以提供关于新闻理解的补充信息。因此，使用多模态内容建模技术有可能提高新闻理解的全面性。第三，新闻建模有很多有用的因素是新闻内容没有涵盖的，例如出版商、流行度和新近度。需要一个统一的框架来整合各种新闻信息（例如，属性特征和上下文特征），同时有效地建模不同特征之间的相关性。对这些方向的进一步研究，有助于更准确、更深入地理解新闻，为后续的用户建模和新闻排名赋能。

### 10.2 全局用户建模

​	用户建模对于了解用户对新闻的兴趣至关重要。然而，对于新闻推荐来说，对动态多样的用户兴趣进行准确、全面的建模是困难的。为了解决这个问题，需要一个可以对各种用户兴趣进行建模的通用用户建模框架。我们认为该框架应满足以下要求。首先，用户建模框架需要从多种用户行为和反馈中综合推断用户兴趣。这是因为点击行为非常嘈杂，对于某些用户来说可能是稀疏的，仅从点击行为来建模用户兴趣是不够的。幸运的是，不同类型的用户行为和反馈（例如，阅读和不喜欢）可以提供丰富的补充信息，例如用户参与，并将它们合并到一个统一的框架中可以更好地支持用户建模。其次，该框架需要对多样化和多粒度的用户兴趣进行建模。由于单个用户嵌入可能不足以全面模拟用户兴趣，因此它可能是一种很有前途的方法，可以用更复杂的结构（例如嵌入集和图）来表示用户兴趣，以提高对用户兴趣的理解。第三，框架需要捕捉用户兴趣的动态。由于用户兴趣通常随时间而变化，因此了解不同时期的用户兴趣并进一步建模它们的内在关系非常重要。为了达到这个目的，使用更先进的序列建模技术可以帮助改进个性化新闻推荐中的用户兴趣建模。

### 10.3  有效高效的个性化排名

​	新闻排名是进行个性化新闻推荐的必要步骤。提高新闻排名主要有三个研究方向。首先，大多数现有的个性化排名方法主要是基于候选新闻和用户兴趣之间的粗粒度相关性，这对于准确定位用户兴趣可能不是最优的。虽然有一些方法可以对用户和新闻之间的细粒度相关性进行建模，但它们效率低下，可能不适合计算资源有限和延迟容忍的场景。因此，开发有效和高效的个性化排名方法对于提高在线新闻推荐非常重要.其次，仅根据相关性对新闻进行排名可能会导致过滤泡沫问题。设计更复杂的新闻排名策略以实现准确性和多样性之间的良好折衷非常重要。第三，现有的新闻排名方法大多是贪心的，即在排名策略中只考虑当前排名。但是，从长远来看，它们可能不是实现良好用户参与度的最佳选择。因此，设计适当的新闻排名策略来优化长期奖励可能有利于用户体验。

### 10.4 新闻推荐的双曲线表示学习

​	在现有的大多数新闻推荐方法中，新闻和用户表示都是在欧几里得空间中学习的。内积和余弦相似度等匹配函数被广泛用于计算新闻排名的相关性分数。然而，欧几里得空间中的表示学习在捕捉数据的层次结构方面是无效的，而双曲线表示学习在这方面要好得多。个性化新闻推荐中存在许多固有的分层数据结构，如不同层次的用户兴趣、新闻主题、知识图谱编码的常识知识。因此，具有双曲线表示学习的新闻推荐可能是一个有前途的解决方案。双曲线空间中有几种现有的神经架构，例如双曲线注意力[60]和双曲线GCN [20]，它们可以作为新闻推荐的核心模型组件。此外，双曲线表示学习在基于 CF 的推荐 [184, 194] 和知识图嵌入 [19, 198] 中也有一些成功的应用，可为新闻推荐中的协作信号建模和知识利用提供有用的指导。未来对双曲线表示学习的研究可能会创造一个新的方向来克服当前在欧几里得空间中进行的用户/新闻建模和个性化排名技术的几个缺点。

### 10.5 统一模型训练

​	模型训练技术对于学习有效且稳健的个性化新闻推荐模型也很重要。未来改进模型训练的工作有四个潜在方向。首先，大多数方法只使用点击信号进行模型训练，这可能是不准确的，因为点击信号通常是嘈杂和有偏差的。此外，特定任务中的监督信号也可能不足 [220]。因此，一个将各种监督和自我监督训练信号和目标结合起来进行协作模型学习的统一框架可以有效地提高模型质量。其次，虽然有几种方法探索使用多任务学习框架将多个目标整合到模型训练中，但它们需要在模型训练中手动调整不同任务的损失系数，这通常需要大量的人力，而且可能对数据集的特征很敏感。因此，自适应多任务学习框架可以自动调整损失系数等超参数，可以减少开发工作量并提高模型的通用性。第三，许多方法使用随机选择的负样本进行模型训练，这可能是嘈杂且信息量少的。因此，使用更有效的负采样可以帮助训练更稳健和准确的新闻推荐模型。第四，在线训练的模型可能与在线场景存在差距，并且可能会随着时间的推移而遭受性能下降的影响。因此，结合在线和在线学习技术以帮助模型更好地适应最新的在线服务需求非常重要。
